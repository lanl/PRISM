...
function fit_inference!(inf_obj::Inference_obj; n_iters=10000, batch_size=100, handoff = 1e20)
    ...
    opt_cmean = [Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.02)  , Flux.Optimise.InvDecay(0.01)) for ii in 1:n_latent]
    opt_lrho  = [Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.0075), Flux.Optimise.InvDecay(0.01)) for ii in 1:n_latent]
    opt_lkap  = [Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.0075), Flux.Optimise.InvDecay(0.01)) for ii in 1:n_latent]
    opt_lsig  = [Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.0075), Flux.Optimise.InvDecay(0.01)) for ii in 1:n_latent]
    optm      = [Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.01),   Flux.Optimise.InvDecay(0.01)) for ii in 1:n_latent]
    optS      = [Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.002),  Flux.Optimise.InvDecay(0.01)) for ii in 1:n_latent]
    optx      = [Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.01),   Flux.Optimise.InvDecay(0.01)) for ii in 1:n_latent]

    # Will optimize Cholesky to ensure positive definite
    trace_elbo = zeros(n_iters)

    batches = vcat([randperm(inf_obj.data.n) for ii in 1:ceil(n_iters*batch_size/inf_obj.data.n)]...)
    ...
    for t = 1:n_iters
        ...
        inds   = batches[((t-1)*batch_size + 1):(t*batch_size)]
...
function fit_svgp!(svgp::SVGP_obj; n_iters=10000, batch_size=100, handoff=1e20,
    ...
    # Initialize optimizers
    opt_cmean = Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.02),   Flux.Optimise.InvDecay(0.01))
    opt_lrho  = Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.0075), Flux.Optimise.InvDecay(0.01))
    opt_lkap  = Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.0075), Flux.Optimise.InvDecay(0.01))
    opt_lsig  = Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.0075), Flux.Optimise.InvDecay(0.01))
    optm      = Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.01),   Flux.Optimise.InvDecay(0.01))
    optS      = Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.002),  Flux.Optimise.InvDecay(0.01))
    optx      = Flux.Optimiser(Flux.ClipValue(grad_clip), ADAM(1. * 0.01),   Flux.Optimise.InvDecay(0.01))
    ...
    batches  = vcat([randperm(svgp.data.n) for ii in 1:ceil(n_iters*batch_size/svgp.data.n)]...)
...
"""
    fit_inference!(inf_obj::Inference_obj, prior_obj::Inference_obj; n_iters=10000, batch_size=100, handoff = 1e20)
Fit SVGP to data and update SVGP_obj in place. 
    ...
    handoff:                 does not work!! Keep larger than n_iters for now
"""
function fit_inference!(inf_obj::Inference_obj, prior_obj::Inference_obj; n_iters=10000, batch_size=100, handoff = 1e20)
    n_latent = length(inf_obj.params)
    ...
    for t = 1:n_iters
        ...
        inds   = rand(1:inf_obj.data.n, batch_size)
        grads  = gradient(gps -> inference_elbo(inf_obj.data.x[inds,:], inf_obj.data.y[inds], inf_obj.data.n, gps, prior_obj), inf_obj)[1]
        ...
        trace_elbo[t]        = inference_elbo(inf_obj.data.x[inds,:], inf_obj.data.y[inds], inf_obj.data.n, inf_obj, prior_obj)
    end
...
"""
    fit_svgp!(svgp::SVGP_obj, prior_obj::SVGP_obj; n_iters=10000, batch_size=100, handoff = 1e20, return_param_traces = false, progress_bar=true)
Fit SVGP to data and update SVGP_obj in place. 
    ...
    progress_bar:            whether or not to show a progress bar (defaults to true)
"""
function fit_svgp!(svgp::SVGP_obj, prior_obj::SVGP_obj; n_iters=10000, batch_size=100, handoff=1e20,
                   return_param_traces=false, progress_bar=true)
    ...
    for t in iterator
        ...
        inds   = rand(1:svgp.data.n, batch_size)
        grads  = gradient(gp -> svgp_elbo(svgp.data.x[inds,:], svgp.data.y[inds], gp, svgp, prior_obj.params), svgp.params)[1][]
        ...
        trace_elbo[t]        = svgp_elbo(svgp.data.x[inds,:], svgp.data.y[inds], svgp.params, svgp, prior_obj.params)
        if return_param_traces
            trace_cmean[t]     = svgp.params.const_mean[1]
            trace_xi[t, :, :]  = svgp.params.inducing_locs
            trace_mn[t, :]     = svgp.params.inducing_mean
